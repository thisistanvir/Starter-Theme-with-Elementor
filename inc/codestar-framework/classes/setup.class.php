...
if (!class_exists('CSF')) {
  class CSF {
    ...
    public static function textdomain() {
      load_textdomain('starter-theme', self::$dir . '/languages/' . get_locale() . '.mo');
    }
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      // Main style
      wp_enqueue_style('starter-theme', self::include_plugin_url('assets/css/style' . $min . '.css'), array(), self::$version, 'all');
      ...
      wp_enqueue_script('starter-theme', self::include_plugin_url('assets/js/main' . $min . '.js'), array('csf-plugins'), self::$version, true);

      // Main variables
      wp_localize_script('starter-theme', 'csf_vars', array(
        'color_palette'     => apply_filters('csf_color_palette', array()),
        'i18n'              => array(
          'confirm'         => esc_html__('Are you sure?', 'starter-theme'),
          'typing_text'     => esc_html__('Please enter %s or more characters', 'starter-theme'),
          'searching_text'  => esc_html__('Searching...', 'starter-theme'),
          'no_results_text' => esc_html__('No results found.', 'starter-theme'),
        ),
      ));
    ...
    public static function field($field = array(), $value = '', $unique = '', $where = '', $parent = '') {
      ...
      if (!empty($field['_notice'])) {
        ...
        $field            = array();
        $field['content'] = esc_html__('Oops! Not allowed.', 'starter-theme') . ' <strong>(' . $field_type . ')</strong>';
        $field['type']    = 'notice';
        $field['style']   = 'danger';
      }
      ...
      if (!empty($field_type)) {
        ...
        if (class_exists($classname)) {
          ...
          echo '<p>' . esc_html__('Field not found!', 'starter-theme') . '</p>';
        }
      } else {
        echo '<p>' . esc_html__('Field not found!', 'starter-theme') . '</p>';
      }
